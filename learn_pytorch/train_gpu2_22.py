...
device = torch.device("cuda")   # 第二种方式   cuda / cuda:0 1 2  (多卡)  /  cpu
#1、准备数据集(训练和测试)
...
EgNet.to(device)   #  第二种gpu调用方式

# 创建损失函数
loss_fn = nn.CrossEntropyLoss()
# if torch.cuda.is_available():
#     loss_fn = loss_fn.cuda()  # 调用GPU
loss_fn.to(device)   #  第二种gpu调用方式
# 定义优化器
...
for i in range(epoch):
    ...
    for data in train_dataloader:
        ...
        imgs = imgs.to(device)   #  第二种调用gpu方式
        targets = targets.to(device)    # 第二种Gpu调用方式
    ...
    with torch.no_grad():   # 不需要设置梯度，只需要测试，不需要对梯度进行调整，更不需要利用梯度来进行优化
        for data in test_dataloader:
            ...
            imgs = imgs.to(device)    #  第二种gpu调用方式
            targets = targets.to(device)   #  第二种gpu调用方式
